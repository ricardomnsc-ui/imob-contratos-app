...
// Grava campos avulsos da compra (hoje: marca que o e-mail de entrega saiu).
async function atualizarCompra(id, patch) {
  if (usingPostgres) {
    await pool.query("UPDATE compras SET data = data || $2 WHERE id = $1", [id, patch]);
    return;
  }
  const all = readJsonFile(comprasFile);
  if (!all[id]) return;
  all[id].data = { ...all[id].data, ...patch };
  writeJsonFile(comprasFile, all);
}
...
module.exports = {
  ...
  addCompra, getCompra, getCompraPorSessao, registrarDownloadCompra, getCompras, atualizarCompra,
};
