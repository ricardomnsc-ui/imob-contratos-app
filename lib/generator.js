...
function buildFactory(branding, themeKey) {
  ...
  function buildCompraVenda(d) {
    ...
    const chavesResumoCV = {
      na_assinatura: `Na assinatura deste contrato${dataChaves ? ` (${dataChaves})` : ""}`,
      ultima_parcela: `Na quitação da última parcela${dataChaves ? ` (${dataChaves})` : ""}`,
      data_fixa: dataChaves || "",
    }[modoChaves] || "";
    ...
    const resumoCV = blocoResumo([
      ...
      ["Comissão de corretagem", corretValor ? `${corret.percentual || 5}% — ${fmtBRL(corretValor)}` : ""],
