...
function buildFactory(branding, themeKey) {
  ...
  function buildCompraVenda(d) {
    ...
    const MOMENTOS_COMISSAO = {
      proporcional:     { texto: "de forma proporcional a cada parcela recebida pelo(s) PROMITENTE(S) VENDEDOR(ES)", curto: "proporcional às parcelas" },
      entrada:          { texto: "integralmente no ato do pagamento da entrada/sinal pelo(s) PROMISSÁRIO(S) COMPRADOR(ES)", curto: "na entrada" },
      primeira_parcela: { texto: "integralmente no ato do pagamento da primeira parcela do preço", curto: "na 1ª parcela" },
      assinatura:       { texto: "integralmente no ato da assinatura deste instrumento", curto: "na assinatura" },
      liberacao_banco:  { texto: "integralmente no ato da liberação do valor financiado pela instituição financeira", curto: "na liberação do financiamento" },
      escritura:        { texto: "integralmente no ato da lavratura da escritura pública de compra e venda", curto: "na escritura" },
      quitacao_total:   { texto: "integralmente quando da quitação integral do preço ajustado", curto: "na quitação integral" },
    };
    ...
    const momentoComissao = momentoId === "outro"
      ? { texto: corret.pagamentoDescricao || PLACEHOLDER, curto: corret.pagamentoDescricao || "" }
      : MOMENTOS_COMISSAO[momentoId];
    ...
    children.push(pMix([
      ...
      { text: `, a ser pago ${momentoComissao.texto}.` },
    ]));
    ...
    children.push(...paragrafo("Parágrafo Primeiro", `O pagamento da comissão deverá ser realizado por meio de ${ondePagar}, em favor da ${(branding.nome || "imobiliária").toUpperCase()}.`));
  ...
  function buildAutorizacaoVenda(d) {
    ...
    children.push(p(`Fica eleito o foro da Comarca de ${branding.foroPadrao || "Natal/RN"} para dirimir questões oriundas deste instrumento.`));

    children.push(localData(d));
    children.push(blocoAssinaturas([[{ nome: prop.nome || " ", papel: "PROPRIETÁRIO(A)" }, { nome: IMOB(), papel: "IMOBILIÁRIA" }]]));
    return children;
  }
  ...
  return {
    ...
    buildFichaVisita, buildAutorizacaoVenda, buildContratoExclusividade,
...
async function gerarContrato(dados, branding) {
  ...
  else if (tipo === "locacao_caucao" || tipo === "locacao_fiador" || tipo === "locacao_seguro_fianca") children = factory.buildLocacao(dados);
