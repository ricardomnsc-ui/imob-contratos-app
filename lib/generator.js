...
function buildFactory(branding, themeKey) {
  ...
  function buildLocacao(d) {
    ...
    children.push(...tituloContrato("CONTRATO DE LOCAÇÃO RESIDENCIAL", `(GARANTIA: ${GARANTIA_LABEL[garantiaTipo] || "CAUÇÃO"})`, { linha2Color: MUTED, linha2Size: 22 }));
    ...
    children.push(cl("DO ALUGUEL E REAJUSTE"));
    children.push(pMix([
    ...
    if (admin.ativa) {
      const taxaAdmin = Number(admin.taxaPercentual !== undefined && admin.taxaPercentual !== "" ? admin.taxaPercentual : 10);
      children.push(cl("DA ADMINISTRAÇÃO DO IMÓVEL"));
      children.push(pMix([
        { text: "A presente locação será administrada pela " },
        { text: `${(branding.nome || "IMOBILIÁRIA").toUpperCase()}${branding.creci ? `, CRECI: ${branding.creci}` : ""}`, bold: true },
        { text: ", que ficará responsável pelo recebimento do aluguel e encargos, repasse ao(à) " },
        { text: "LOCADOR(A)", bold: true },
        { text: " e prestação de contas mensal, mediante taxa de administração de " },
        { text: `${taxaAdmin}% (${taxaAdmin} por cento) sobre o valor do aluguel mensal`, bold: true },
        { text: ", a ser descontada no momento do repasse." },
      ]));
    }
    ...
    if (garantiaTipo === "fiador") {
      ...
      children.push(p(`Como garantia da presente locação, o(a) LOCATÁRIO(A) deposita a importância de ${fmtBRL(g.valor)} a título de caução, a ser restituída ao final da locação, descontados eventuais débitos.`));
    }

    const corretLoc = d.corretagem || {};
    const temCorretLoc = corretLoc.percentual !== undefined && corretLoc.percentual !== "";
    if (temCorretLoc) {
      const qtdAlugueis = Number(corretLoc.quantidadeAlugueis || 1);
      const pctLoc = Number(corretLoc.percentual);
      const aluguelValor = Number((d.aluguel || {}).valor || 0);
      const corretValorLoc = corretLoc.valor !== undefined && corretLoc.valor !== "" ? Number(corretLoc.valor) : aluguelValor * qtdAlugueis * (pctLoc / 100);
      const qtdTxt = qtdAlugueis === 1 ? "do 1º (primeiro) aluguel" : `dos ${qtdAlugueis} primeiros aluguéis`;
      children.push(cl("DA INTERMEDIAÇÃO/CORRETAGEM"));
      children.push(pMix([
        { text: "O presente negócio é feito sob a intermediação da " },
        { text: `${(branding.nome || "IMOBILIÁRIA").toUpperCase()}${branding.creci ? `, CRECI: ${branding.creci}` : ""}`, bold: true },
        { text: `${branding.cnpj ? `, inscrita no CNPJ nº ${branding.cnpj}` : ""}, que receberá do(a) ` },
        { text: "LOCADOR(A)", bold: true },
        { text: ", a título de corretagem pelos serviços de intermediação ora prestados, o valor correspondente a " },
        { text: `${pctLoc}% (${pctLoc} por cento) ${qtdTxt}, equivalente a ${fmtBRL(corretValorLoc)}`, bold: true },
        { text: "." },
      ]));
    }

    children.push(cl("DA ENTREGA DAS CHAVES"));
    const ec = d.entregaChaves || {};
    let textoChaves;
