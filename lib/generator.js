...
function buildFactory(branding, themeKey) {
  ...
  function buildCompraVenda(d) {
    ...
    const corretResumo = corretPorValor
      ? fmtBRL(corretValor)
      : `${percNumeral}% — ${fmtBRL(corretValor)}`;
    ...
    children.push(...paragrafo("Parágrafo Primeiro", `O pagamento da comissão deverá ser realizado por meio de ${ondePagar}, em favor da ${(branding.nome || "imobiliária").toUpperCase()}, de forma proporcional ao valor recebido pelo(s) PROMITENTE(S) VENDEDOR(ES).`));
