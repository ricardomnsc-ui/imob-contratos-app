...
function qualificacao(p) {
  ...
  partes.push(`portador(a) do RG nº ${need(p.rg)}`);
...
function buildFactory(branding) {
  ...
  function p(text, opts = {}) {
    return new Paragraph({
      ...
      children: [new TextRun({ text, font: "Calibri", size: opts.size || 20, bold: !!opts.bold, italics: !!opts.italics, color: opts.color || DARK })],
    });
  }

  function pMix(parts, opts = {}) {
    const runs = parts.map(part => new TextRun({
      text: part.text, font: "Calibri", size: opts.size || 20,
  ...
  function clausula(num, titulo) {
    ...
    return new Paragraph({
      ...
      children: [new TextRun({ text: `CLÁUSULA ${num} – ${titulo}`, bold: true, font: "Calibri", size: 22, color: DARK })],
    });
  }
  ...
  function paragrafo(label, texto) {
    return [
      new Paragraph({
        ...
        indent: { left: 360 },
        children: [new TextRun({ text: label, bold: true, font: "Calibri", size: 20, color: DARK })],
      }),
      new Paragraph({
        ...
        children: [new TextRun({ text: texto, font: "Calibri", size: 20, color: DARK })],
      }),
    ];
  }
  ...
  function blocoParte(rotulo, pessoas) {
    ...
    filhos.push(new Paragraph({
      spacing: { after: 80 },
      children: [new TextRun({ text: rotulo, bold: true, font: "Calibri", size: 18, color: GREEN })],
    }));
    pessoas.forEach((pe, i) => {
      if (i > 0) filhos.push(new Paragraph({ spacing: { after: 60 }, children: [new TextRun({ text: " ", size: 14 })] }));
      filhos.push(new Paragraph({
        spacing: { after: 60 },
        children: [new TextRun({ text: pe.nome, bold: true, font: "Calibri", size: 22, color: DARK })],
      }));
      filhos.push(new Paragraph({
        alignment: AlignmentType.JUSTIFIED,
        spacing: { after: 0, line: 280 },
        children: [new TextRun({ text: qualificacao(pe), font: "Calibri", size: 20, color: DARK })],
      }));
    });
  ...
  function tabelaPagamentos(parcelas, totalLabel, totalValor) {
    const headerCell = (text) => new TableCell({
      ...
      margins: cellMargins,
      children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text, bold: true, font: "Calibri", size: 20, color: "FFFFFF" })] })],
    });
    const dataCell = (text, opts = {}) => new TableCell({
      borders: tBorders,
      shading: { fill: opts.row ? ROW : "FFFFFF", type: ShadingType.CLEAR, color: "auto" },
      margins: cellMargins,
      children: [new Paragraph({ alignment: opts.align || AlignmentType.CENTER, children: [new TextRun({ text, font: "Calibri", size: 20, bold: !!opts.bold, color: DARK })] })],
    });
    ...
    rows.push(new TableRow({
      children: [
        new TableCell({
          ...
          children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: totalLabel || "TOTAL", bold: true, font: "Calibri", size: 20, color: "FFFFFF" })] })],
        }),
        new TableCell({
          borders: tBorders, shading: { fill: HEADER_FILL, type: ShadingType.CLEAR, color: "auto" }, margins: cellMargins,
          children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: fmtBRL(totalValor), bold: true, font: "Calibri", size: 20, color: "FFFFFF" })] })],
        }),
      ],
    }));
  ...
  function blocoCaixa(linhas) {
    const filhos = linhas.map(l => new Paragraph({
      ...
      children: [
        new TextRun({ text: l[0], bold: true, font: "Calibri", size: 20, color: DARK }),
        new TextRun({ text: l[1] || "", font: "Calibri", size: 20, color: DARK }),
      ],
    }));
    const cell = new TableCell({
      borders: {
  ...
  function blocoAssinaturas(pares) {
    const sigCell = (nome, papel) => new TableCell({
      ...
      children: [
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: "ASSINATURA DIGITAL", font: "Calibri", size: 14, color: MUTED })] }),
        new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: " ", size: 18 })] }),
        new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: " ", size: 18 })] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: "____________________________________", font: "Calibri", size: 18, color: DARK })] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: nome, bold: true, font: "Calibri", size: 20, color: DARK })] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0 }, children: [new TextRun({ text: papel, font: "Calibri", size: 16, color: MUTED })] }),
      ],
    });
  ...
  function buildHeader() {
    ...
    if (branding.logoBuffer) {
      ...
      const infoCell = new TableCell({
        borders: noBorders, margins: { top: 0, bottom: 0, left: 0, right: 0 },
        width: { size: 4535, type: WidthType.DXA }, verticalAlign: VerticalAlign.CENTER,
        children: infoLines.map(t => new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { after: 0 }, children: [new TextRun({ text: t, font: "Calibri", size: 18, color: MUTED })] })),
      });
      ...
      children.push(new Paragraph({
        alignment: AlignmentType.LEFT, spacing: { after: 0 },
        children: [
          new TextRun({ text: (branding.nome || "").toUpperCase(), bold: true, font: "Calibri", size: 24, color: DARK }),
          new TextRun({ text: infoLines.length ? "   " + infoLines.join("  |  ") : "", font: "Calibri", size: 16, color: MUTED }),
        ],
      }));
    }
    children.push(new Paragraph({
  ...
  function buildFooter() {
    ...
    const footerLine = new Paragraph({
      ...
      children: [
        new TextRun({ text: (branding.nome || "").toUpperCase(), bold: true, font: "Calibri", size: 14, color: DARK }),
        new TextRun({ text: `\t${branding.endereco || ""}`, font: "Calibri", size: 14, color: MUTED }),
        new TextRun({ text: `\t${[branding.email, branding.creci ? `CRECI: ${branding.creci}` : null].filter(Boolean).join("  |  ")}`, font: "Calibri", size: 14, color: MUTED }),
      ],
    });
    return new Footer({ children: [greenLine, footerLine] });
  }
  ...
  function buildCompraVenda(d) {
    ...
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 240, after: 80, line: 300 }, children: [new TextRun({ text: "CONTRATO PARTICULAR DE PROMESSA", bold: true, font: "Calibri", size: 28, color: DARK })] }));
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 320, line: 300 }, children: [new TextRun({ text: "DE COMPRA E VENDA DE IMÓVEL", bold: true, font: "Calibri", size: 28, color: DARK })] }));
    ...
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 160, after: 160 }, children: [new TextRun({ text: "— E de outro —", italics: true, font: "Calibri", size: 20, color: MUTED })] }));
    ...
    children.push(clausula("DÉCIMA", "DO FORO"));
    ...
    children.push(new Paragraph({ spacing: { before: 320, after: 80 }, border: { top: { style: BorderStyle.SINGLE, size: 12, color: GREEN, space: 4 } }, children: [new TextRun({ text: "" })] }));
    children.push(new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { before: 60, after: 320 }, children: [new TextRun({ text: `${(branding.cidade || "Natal")}, ${fmtData(d.data)}`, bold: true, font: "Calibri", size: 22, color: DARK })] }));
    ...
    children.push(new Paragraph({ spacing: { before: 200, after: 80 }, children: [new TextRun({ text: "TESTEMUNHAS:", bold: true, font: "Calibri", size: 20, color: DARK })] }));
    const testCell = () => new TableCell({
      ...
      children: [
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: "ASSINATURA DIGITAL", font: "Calibri", size: 14, color: MUTED })] }),
        new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: " ", size: 18 })] }),
        new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 80 }, children: [new TextRun({ text: "____________________________________", font: "Calibri", size: 18, color: DARK })] }),
        new Paragraph({ spacing: { after: 40 }, children: [new TextRun({ text: "Nome: ________________________________", font: "Calibri", size: 18, color: MUTED })] }),
        new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: "CPF: __________________________________", font: "Calibri", size: 18, color: MUTED })] }),
      ],
    });
  ...
  function buildLocacao(d) {
    ...
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 240, after: 80, line: 300 }, children: [new TextRun({ text: "CONTRATO DE LOCAÇÃO RESIDENCIAL", bold: true, font: "Calibri", size: 28, color: DARK })] }));
    children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 320, line: 300 }, children: [new TextRun({ text: ehFiador ? "(GARANTIA: FIADOR)" : "(GARANTIA: CAUÇÃO)", bold: true, font: "Calibri", size: 22, color: MUTED })] }));
    ...
    if (ehFiador && d.fiadores && d.fiadores.length > 0) {
      children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 160, after: 160 }, children: [new TextRun({ text: "— E ainda —", italics: true, font: "Calibri", size: 20, color: MUTED })] }));
    ...
    children.push(pMix([
      ...
      { text: ` o imóvel localizado na ${need(d.imovel && d.imovel.endereco)}${d.imovel && d.imovel.caracteristicas ? `, ${d.imovel.caracteristicas}` : ""}, destinado exclusivamente a uso ${d.uso || "residencial"}.` },
    ]));

    children.push(clausula("SEGUNDA", "DO PRAZO"));
    children.push(p(`A presente locação vigorará pelo prazo de ${need(d.vigencia && d.vigencia.meses, "30")} meses, com início em ${fmtDataCurta((d.vigencia||{}).inicio)} e término em ${fmtDataCurta((d.vigencia||{}).fim)}, prorrogando-se por prazo indeterminado caso nenhuma das partes manifeste interesse contrário.`));

    children.push(clausula("TERCEIRA", "DO ALUGUEL E REAJUSTE"));
    children.push(pMix([
      { text: "O valor mensal do aluguel é de " },
      { text: `${fmtBRL((d.aluguel||{}).valor)}${(d.aluguel||{}).extenso ? ` (${d.aluguel.extenso})` : ""}`, bold: true },
      { text: `, com vencimento todo dia ${need((d.aluguel||{}).diaVencimento, "10")} de cada mês. O aluguel será reajustado anualmente pelo índice ${need((d.aluguel||{}).indice, "IGP-M")}.` },
    ]));

    children.push(clausula("QUARTA", "DA GARANTIA"));
    if (ehFiador) {
      ...
      children.push(p(`Como garantia da presente locação, o(a) LOCATÁRIO(A) deposita a importância de ${fmtBRL((d.garantia||{}).valor)} a título de caução, a ser restituída ao final da locação, descontados eventuais débitos.`));
    }

    children.push(clausula("QUINTA", "DA ENTREGA DAS CHAVES"));
    const ec = d.entregaChaves || {};
    let textoChaves;
    if (ec.modo === "na_assinatura") textoChaves = `As chaves serão entregues na data de assinatura deste instrumento${ec.data ? ` (${fmtDataCurta(ec.data)})` : ""}.`;
    else if (ec.modo === "data_fixa") textoChaves = `As chaves serão entregues em ${fmtDataCurta(ec.data) || PLACEHOLDER}, conforme acordado entre as partes.`;
    else textoChaves = `As chaves serão entregues conforme: ${ec.descricao || PLACEHOLDER}.`;
    children.push(p(textoChaves));

    children.push(clausula("SEXTA", "DAS DESPESAS"));
    children.push(p("Caberá ao(à) LOCATÁRIO(A) o pagamento de todas as despesas relativas ao consumo de água, energia elétrica, gás, internet, IPTU e taxas condominiais, durante a vigência do presente contrato."));

    children.push(clausula("SÉTIMA", "DA MULTA"));
    children.push(p("O descumprimento de qualquer cláusula deste contrato implicará multa equivalente a 3 (três) aluguéis vigentes, sem prejuízo das demais cominações legais."));

    children.push(clausula("OITAVA", "DO FORO"));
    children.push(p(`As partes elegem o foro da Comarca de ${need(d.foro, branding.foroPadrao || "Natal/RN")} como o competente para dirimir qualquer lide decorrente deste contrato.`));
...
/**
 * @param {object} dados - dados do contrato (schema em references/data-schema.md)
...
async function gerarContrato(dados, branding) {
  const factory = buildFactory(branding || {});
