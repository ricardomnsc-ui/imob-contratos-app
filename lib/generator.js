...
function buildFactory(branding, themeKey) {
   ...
   * com as condições principais da locação, para leitura rápida antes das
   * cláusulas. Linhas sem valor são omitidas.
   */
  function blocoResumo(linhas) {
    const rows = linhas
      .filter(l => l && l[1] !== undefined && l[1] !== null && String(l[1]).trim() !== "")
      .map(([rotulo, valor]) => new TableRow({
        children: [
          ...
          new TableCell({
            ...
            children: [new Paragraph({
              spacing: { line: 260 },
              children: [new TextRun({ text: String(valor), font: FONT_CORPO, size: 19, color: DARK })],
            })],
          }),
        ],
      }));
