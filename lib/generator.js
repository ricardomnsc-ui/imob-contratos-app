...
function buildFactory(branding, themeKey) {
  ...
  function buildCompraVenda(d) {
    ...
    const prazoComissao = Math.max(0, Math.floor(Number(corret.prazoDias === undefined ? 5 : corret.prazoDias) || 0));
    const emAte = `em até ${prazoComissao} (${numeroPorExtenso(prazoComissao)}) dias corridos contados`;
    ...
    if (momentoId === "outro") {
      ...
      curtoMomento = m.curto + (prazoComissao > 0 ? ` (até ${prazoComissao} dia(s))` : "");
    }
